      ...
      N=10000       ! size of adapting population
      sd=0.2d+0/N
...
! v1:10^5 histories, v2: 10^6 histories      
      ...
      do k3=1,histories
       ...
       x=sqrt(-2.0d+0*log(x1))       
       fitness(2*i-1)=sd*x*dcos(2.0d+0*Pi*x2)
       fitness(2*i)=sd*x*dsin(2.0d+0*Pi*x2)
       if (fitness(2*i-1)>tempg)then
       tempg=fitness(2*i-1)
       endif  
       if (fitness(2*i)>tempg)then
       tempg=fitness(2*i)
       endif  
       enddo   
       tempg=tempg/sd
...
! measure moments of trait value distribution
 ...
 22     zfinal(k3,1)=x1
       ...
       open(4,file='N010000Nsd0.2B.txt')
       do i=1,tsteps/100
 ...
 94    Format(I8,1X,F11.8,1X,F11.8,1X,F11.8,1X,F11.8,1X,F14.8,1X,F11.8,
     c 1X,F11.8,1X,F11.8)
       ...
       open(4,file='N010000Nsd0.2zfinalB.txt')
       do i=1,histories
